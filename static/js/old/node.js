Vespy.Node = function(other, page) {
	if(other) {
		this.I = typeof other.I !== 'undefined' ? _.clone(other.I) : ['_above','_left'];
		this.ID = typeof other.ID !== 'undefined' ? _.clone(other.ID) : makeid();
		this.P = typeof other.P !== 'undefined' ? jsonClone(other.P) : undefined;
		this.V = typeof other.V !== 'undefined' ? _.clone(other.V) : [];
		this.selected = typeof other.selected !== 'undefined' ? _.clone(other.selected) : false;
		this.position = typeof other.position !== 'undefined' ? _.clone(other.position) : [1,1];
		this.type = typeof other.type !== 'undefined' ? _.clone(other.type) : undefined;
		this.executed = typeof other.executed !== 'undefined' ? _.clone(other.executed) : false;
		this.next = typeof other.next !== 'undefined' ? _.clone(other.next) : [];
		this.page = page;
	} else {
		this.I= ['_above','_left'];
		this.ID= makeid();
...
// CLEAN UP NODE FOR SAVING ON SERVER 
Vespy.Node.prototype.serializable = function() { 
	var _this = {};
	_this.I = _.clone(this.I);
	_this.ID = _.clone(this.ID);
	_this.P = _.clone(this.P);
	_this.V = []; 		// IMPORTANT TO MAKE V EMPTY
	_this.selected = false;
	_this.position = _.clone(this.position);
	_this.type = _.clone(this.type);
	_this.executed = false;
	_this.next = [];
	return _this;
};
Vespy.Node.prototype.duplicate = function() { 
	var newNode = new Vespy.Node(this, this.page);
	newNode.ID = makeid();
	newNode.selected = false;
	return newNode;
};
Vespy.Node.prototype.execute = function() { 
	console.log("Executing "+this.P.type);
	Vespy.planner.execute(this);
};
Vespy.Node.prototype.draw = function(targetEl) {
	// NODE BASE
	...
	$(el).disableSelection().css({
		'top':Vespy.grid.p2c(this.position)[0],
		'left':Vespy.grid.p2c(this.position)[1],
		'width':this.node_size,
		'height':this.node_size
	}).appendTo(targetEl);
};

Vespy.Node.prototype.getNodeValueTable = function() {
	...
	_.each(this.V.slice(0,num_v_to_draw), function(v,vi,list) {
...
Vespy.Node.prototype.getParamNodeID = function(paramKey) {
...
Vespy.Node.prototype.getParamValue = function(paramKey) {
	var node_id = this.getParamNodeID(paramKey);
	if(node_id)	return Vespy.page.get_node_by_id(node_id,node).V;
	else return str2value(node.P.param[paramKey]);
};
Vespy.Node.prototype.getNodeIcon = function() {
	var icon;
