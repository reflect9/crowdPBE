class TaskDefinition {
    ...
    public example: Trial;
    public addStep: boolean;
    public addCase: boolean;
	public solution: any[][];
    public testOutput: Function;

    constructor (tid: string) {
        let data: Object = TaskDefinition.pbeTasks[tid];
    ...
    public static getTaskDefinition(tid: string) {
        return new TaskDefinition(tid);
    }
    public static getAllTaskIDs(): string[] {
        return _.filter(_.keys(TaskDefinition.pbeTasks), function(k){return k.indexOf("task")!=-1; });
    }
    public static getAllTutorialIDs(): string[] {
	...
	public static getIndexFromTID(tid: string): number {
		return _.indexOf(this.getAllIDs(), tid);
	}

	private static pbeTasks = {
		'tutorial_1': {
			'taskID':'tutorial_1',
			...
			'testOutput':function(data){
				try {
					var inputValues: number[] = _.flatten(_.map(data['input'], function(v){ return str2arrayNumber(v); }));
					var outputValues: number[] = _.flatten(_.map(data['output'], function(v){ return str2arrayNumber(v); }));	
