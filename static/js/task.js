	...
	$(taskEl).find("button.teachButton").click(function(event, ui){
		...
		if(isEveryStepRight){
			...
			if(programTestResult.isValid == true) {
		      ...
		      $(taskEl).find(".testResult").html("<span style='color:red;'>"+MESSAGE_EXAMPLES_INCONSISTENT_WITH_PROGRAM+"</span>");;
		    }
	...
	for(var i=0; i<num_prog_list.length; i++){
		...
		if(minimum_num_prog==0) {  // NO PROGRAM IS GENERATED FOR THE STEP
			if(mode == MODE_EXPERIMENTAL) {
				...
				var failAnalysis = analyzeFailure(data, i, tr);
				$(lastColumn).html(failAnalysis.html);
				Log.push({
			    	event:"FAIL_MESSAGE",
			    	tid:tid,
			    	detail:{
...
function analyzeFailure(data, i, tr) {
