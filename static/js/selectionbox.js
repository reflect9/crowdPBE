...
MixedVespy.SelectionBox = function(edgeSize, edgeColor, edgeContainer) {
	...
	var getWindowScrollSize = function() {
		...
		// then we need to check if Vespy.documentBody has a bigger scroll size value
		// because sometimes depending on the browser and the page, the Vespy.documentBody
		// scroll size returns a smaller (and wrong) measure
		if ((el = Vespy.documentBody) && (el.scrollHeight || el.scrollWidth) &&
	...
	var getWindowSize = function() {
		...
		else if ((el = Vespy.documentBody) && (el.clientHeight || el.clientWidth)) {
		width = el.clientWidth;
		height = el.clientHeight;
		}
		return {width: width, height: height};
	...
	var getWindowScrollPosition = function() {
		...
		else if ((el = Vespy.documentBody) && (el.scrollTop || el.scrollLeft)) {
		top = el.scrollTop;
		left = el.scrollLeft;
		}
